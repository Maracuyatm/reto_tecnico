...
public class App {
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            ...
            // Comprobar si se trata de un filtro
            String[] data = input.split("\\|");
            if (data.length == 2 && data[0].equalsIgnoreCase("filtrar")) {
                // Filtrar animales por tipo
                try {
                    System.out.println("Filtrando por tipo: " + data[1]);  // Depuración aquí

                    // Convertir el tipo a Enum (debe coincidir con el nombre del enum)
                    TipoAnimal tipoFiltro = TipoAnimal.valueOf(data[1].toUpperCase());
                    ...
                    System.out.println("Tipo de animal no válido: " + data[1]);
                }
                return;  // Detener el proceso después de filtrar
            }

            // Comprobar si se trata de una creación de un nuevo animal
            if (data.length == 3) {
                try {
                ...
                System.out.println("Entrada incorrecta. Debe ser: nombre|tipo|onomatopeya o filtrar|tipo");
            }
        } else {
